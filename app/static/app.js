...
const factibilityModule = window.FactibilityModule.create({
  getApplicationShell: () => applicationShell,
  shellState: State,
  byId: $,
  api,
  escape: esc,
  ...
  formatTableDate,
  toast,
  openSalesSheet: openFactibilitySalesSheet,
  saveUserCache,
  stopOtherModuleSync: stopLiveCandidateSync,
  roleLabels: ROLE_LABEL,
});
const stopFactibilitySync = factibilityModule.stop;
const renderFactibilityLocations = factibilityModule.render;
const factibilityLocationHeading = factibilityModule.locationHeading;
...
async function openFactibilitySalesSheet(candidateId) {
  const item = factibilityModule.findLocation(candidateId);
...
async function downloadProjectSheetFromForm() {
  ...
  if (form.dataset.mode === "factibility") {
    ...
    try {
      ...
      factibilityModule.updateSalesSheet(candidateId, saved);
      await openProjectSheetPreview(
        candidateId,
        `/factibilidad/locations/${candidateId}/sales-sheet.pdf`,
      );
...
function wireInputs() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (factibilityModule.handleEscape(e.key)) return;
...
const startFactibilityApp = factibilityModule.start;
