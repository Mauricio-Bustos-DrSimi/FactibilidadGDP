...
TEST_USERS = [
    ("Cora Coordinator", "coordinator@test.local", "test1234", "coordinator"),
    ("Marco Manager",    "manager@test.local",     "test1234", "manager"),
    ("Dina Director",    "director@test.local",    "test1234", "director"),
    ("Sam Sysadmin",     "sysadmin@test.local",    "test1234", "sysadmin"),
]


def seed_users(db) -> None:
    for name, email, pw, role in TEST_USERS:
...
def main() -> None:
    ...
    try:
        ...
        seed_business(db)
        print("\nDone. Log in at http://127.0.0.1:8011 with any of:")
        for _, email, pw, role in TEST_USERS:
